...
class Collector:
    ...
    def _process_article(
        ...
        *,
        ...
        # "[고병권의 묵묵]피해자가 될 수 없는 사람들"). 이 칼럼들은 briefing.py의 선정 단계에서
        # 토픽 분류와 무관하게 항상 III절 후보로 취급되므로, 그 시점에 실제로 본문을 읽은 기록이
        # 남아 있도록 여기서 title-only 판정과 무관하게 본문 확인을 강제한다.
        mandatory_column = is_mandatory_opinion_column(
            discovery.source,
            " ".join(
                value for value in (discovery.title, discovery.byline, discovery.section) if value
            ),
        )
