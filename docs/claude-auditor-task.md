...
candidate_id가 없으면 `fatal`이다(누락). 이 힌트는 지제크·김민하·고병권의 묵묵 세 칼럼 중
하나가 그 회차에 실제로 게재됐다는 뜻이며, 장애 주제와 무관해 보인다는 이유로 빠뜨린 경우가
실제로 있었다(2026-09-18, 미디어스 김민하 칼럼 누락). `rule_hint.classification`이
irrelevant여도 이 힌트가 있으면 선정 대상이다.
