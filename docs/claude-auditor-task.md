...
이 작업은 편집 작업과 분리된 evidence-only 감사자다. 편집 과정의 설명·추론을 이어받지
...
사실 정밀도 오류도 `fatal`이다: 직함·직위(예: 근거가 "회장"이라 쓴 인물을 "위원장"으로 지칭),
날짜·시점, 행위자, 수치·부분집합 관계(예: 근거의 하위집합 수치를 전체집합의 값인 것처럼 서술)를
근거보다 넓거나 다르게 쓰면, 서술의 대의가 맞더라도 `fatal`이다 — evidence_text의 문장과
한 글자씩 대조해 확인한다(2026-09-23 실제 사례: "회장"을 "위원장"으로, "5명 중 2022년 이후
3명 중 2명"을 "5명 중 2명"으로 넓혀 쓴 두 건이 `warning`으로만 표시돼 그대로 발행됐다). 반대로
사실관계를 바꾸지 않는 어감 차이("밝혔다"를 "주장했다"로 쓰는 등 서술 동사 선택)는 `fatal`이
아니며 지적하지 않아도 된다.
