# Claude 감독 작업 (보관 — 현재 예약 트리거 없음)

별도의 10:00 감독 예약이 있던 시기의 문서다. 현재 일간 파이프라인은 대기열 확보부터
편집·감사·발행 확인까지 하나의 예약 트리거가 순서대로 수행하므로, 이 문서가 설명하는 별도
감독 단계는 지금 실행되지 않는다. 감독 절차를 다시 예약할 때 참고용으로 남겨 둔다.
