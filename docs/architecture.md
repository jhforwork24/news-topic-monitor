...
발행만 전날 07:00 KST부터 당일 05:00 KST까지의 전환 구간을 쓴다). 구간의 시작은 직전 발행일의
05:00 KST 경계다. 예약 발행이 KST 화~토라 월요일 아침 발행이 없으므로, 화요일 구간만 토요일
경계까지 내려가 72시간이 되고(토·일 보도 포함) 나머지 요일은 24시간이다. 창이 길어진 날에는
후보·본문 확인 상한도 창 길이에 비례해 올라가지만 섹션별 이슈 상한은 그대로여서, 늘어난 후보
가운데 무엇을 싣느냐만 편집·감사 단계가 판단한다. I절은
...
여부·요약 길이)을 함께 반영해 최대 3건까지 붙인다. CRPD 조항 연결, 단체 공식입장 원문 확인,
현행법/발의안 구분, 정책연구자료 매핑은 매주 새로 확인해야 하는 자료라 일간 파이프라인에서는
다루지 않고 주간 브리핑에서 수행한다. 서로 다른 매체가 2개 이상 포함된 이슈의 논조 비교는
후보의 `evidence_text`(본문이 확보된 경우 요약이 아니라 실제 본문)를 근거로 최소 2가지
관찰 가능한 요소를 비교하도록 편집·감사 지침에 명시돼 있으며, 매체 진영(보수·진보·전문지)은
실제 텍스트를 읽고 확인한 차이를 묶어 설명하는 용도로만 허용하고 본문을 읽지 않은 채 매체
...
Asia/Seoul로 변환한다. 05:20 백필, 06:17 최종 사전 수집, 09:02 결정론적 보고, 07:34 private
대기열 생성, 09:25 연결형 편집, 09:38 독립 감사, 09:48 final-state·gate·최종 발행, 10:00 감독을
순서대로 실행한다. 대기열 생성의 cron 분값(34분)은 GitHub의 매시 정각 부근 예약 트리거 지연을
피하기 위해 정각에서 의도적으로 떨어뜨린 값이다. GitHub 데이터 작성 작업은 같은 concurrency
그룹으로 직렬화한다. 3시간 수집이 최근 6시간을 겹쳐 보고
